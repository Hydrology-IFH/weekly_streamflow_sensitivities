...
# - changed colors to ceiling(...) so that last catchment is also plotted
...
par( mfrow = c(3,2),mar=c(c(3, 4, 1, 1) + 0.1), cex=0.9)
...
legend(1,ylim1[2], seq(500,3000,length.out=11),col=col, fill=seq(1,25,length.out=11), horiz=F, title="Elevation [m a.s.l.]", bty="n", xjust=-0.1, cex=0.6)
...
ylim1 <- c(0,1)
plot(xx,tcoef[1,], typ="l", ylim=ylim1, col=col[(height[1]-500)*.01], xlab="", ylab="Explained Variance [-]", xaxt = "n", lwd = 2,  xaxs="i" , yaxs="i")
axis(side = 1, at = xtick+13, labels = xtlab, lwd.ticks=0)
axis(side = 1, at = xtick, labels = rep("",13))
abline(0,0)
for (i in 1:nr) {points(xx,tcoef[i,], typ="l",col=col[ceiling((height[i]-500)*.01)], lwd = 2)}
...
ylim1 <- c(0,1)
plot(xx,pcoef[1,], typ="l", ylim=ylim1, col=col[(height[1]-500)*.01], xlab="", ylab="Explained Variance [-]", xaxt = "n", lwd = 2 ,  xaxs="i", yaxs="i")
axis(side = 1, at = xtick+13, labels = xtlab, lwd.ticks=0)
axis(side = 1, at = xtick, labels = rep("",13))
abline(0,0)
for (i in 1:nr) {points(xx,pcoef[i,], typ="l",col=col[ceiling((height[i]-500)*.01)], lwd = 2)}
...
######
# Storage (lag-1) effect

# changelog:
# - save as pdf
# - change layout, axes, legend
...
pdf(file="D:/R/Climate_sensitivity/Figure S1.pdf",width=9, height=8)

par( mfrow = c(2,2),mar=c(c(3, 4, 1, 1) + 0.1), cex=0.9)
for (i in 1:nr) {tcoef[i,] <- ma.filtc(as.numeric(out_Q.coeff[i,]),movavg)}   # relative sensitivity
for (i in 1:nr) {pcoef[i,] <- ma.filtc(as.numeric(out_Q.coeff[i,]/out_Q.p[i,]*100),movavg)}
# 
ylim1 <- c(0,1.5)
plot(xx,tcoef[1,], typ="l", ylim=ylim1, col=col[(height[1]-500)*.01], xlab="", ylab = expression(b[Q]*" [-]"), xaxt = "n", lwd = 2,  xaxs="i" )
axis(side = 1, at = xtick+13, labels = xtlab, lwd.ticks=0)
axis(side = 1, at = xtick, labels = rep("",13))
abline(0,0)
for (i in 1:nr) {points(xx,tcoef[i,], typ="l",col=col[ceiling((height[i]-500)*.01)], lwd = 2)}
...
# 
ylim1 <- c(0,250)
plot(xx,pcoef[1,], typ="l", ylim=ylim1, col=col[(height[1]-500)*.01], xlab="", ylab = expression(S[Q]*" [%/(mm/d)]"), xaxt = "n", lwd = 2 ,  xaxs="i")
axis(side = 1, at = xtick+13, labels = xtlab, lwd.ticks=0)
axis(side = 1, at = xtick, labels = rep("",13))
abline(0,0)
for (i in 1:nr) {points(xx,pcoef[i,], typ="l",col=col[ceiling((height[i]-500)*.01)], lwd = 2)}
...
for (i in 1:nr) {pcoef[i,] <- ma.filtc(as.numeric(out_Q.var[i,]),movavg)}
...
ylim1 <- c(0,1)
plot(xx,clim_coef[1,], typ="l", ylim=ylim1, col=col[(height[1]-500)*.01], xlab="", ylab="Explained Variance Total [-]", xaxt = "n", lwd = 2 ,  xaxs="i", yaxs="i")
axis(side = 1, at = xtick+13, labels = xtlab, lwd.ticks=0)
axis(side = 1, at = xtick, labels = rep("",13))
abline(0,0)
...
# # proportion of temperature sensitivity of streamflow
# 
# for (i in 1:nr) {tcoef[i,] <- ma.filtc(as.numeric(out_T.var[i,]),movavg)}  
# for (i in 1:nr) {pcoef[i,] <- ma.filtc(as.numeric(out_P.var[i,]),movavg)}
# 
# ylim1 <- c(0,1)
# plot(xx,tcoef[1,]/(tcoef[1,]+pcoef[1,]), typ="l", ylim=ylim1, col=col[(height[1]-500)*.01], xlab="", ylab="Proportion T Sensitivity [-]", xaxt = "n", lwd = 2,  xaxs="i" )
# axis(side = 1, at = xtick+13, labels = xtlab, lwd.ticks=0)
# axis(side = 1, at = xtick, labels = rep("",13))
# for (i in 1:nr) {points(xx,tcoef[i,]/(tcoef[i,]+pcoef[i,]), typ="l",col=col[(height[i]-500)*.01], lwd = 2)}
# 
# 
# ylim1 <- c(0,1)
# plot(xx,(tcoef[1,]+pcoef[1,]), typ="l", ylim=ylim1, col=col[(height[1]-500)*.01], xlab="", ylab="Explained Variance of T and P [-]", xaxt = "n", lwd = 2,  xaxs="i" )
# axis(side = 1, at = xtick+13, labels = xtlab, lwd.ticks=0)
# axis(side = 1, at = xtick, labels = rep("",13))
# for (i in 1:nr) {points(xx,(tcoef[i,]+pcoef[i,]), typ="l",col=col[(height[i]-500)*.01], lwd = 2)}
# legend(183,ylim1[2], seq(500,3000,length.out=6),col=col, fill=seq(1,25,length.out=6), horiz=TRUE, title="Elevation [m a.s.l.]", bty="n", xjust=0.5, cex=0.8)
...
pdf(file="D:/R/Climate_sensitivity/Figure S2ab.pdf",width=8, height=4)

par( mfrow = c(1,2),mar=c(c(4, 4, 2, 1) + 0.1), cex=0.8, xaxs="i", yaxs="i")

plot(xx,mod_T$r2, ylim=c(0,1.15), typ="l", lwd=2, xlab="", ylab="Explained Variance [-]", xaxt = "n" )
axis(side = 1, at = xtick+13, labels = xtlab, lwd.ticks=0)
axis(side = 1, at = xtick, labels = rep("",13))
...
plot(xx,mod_P$r2, ylim=c(0,1.15), typ="l", lwd=2, xlab="", ylab="Explained Variance [-]", xaxt = "n" )
axis(side = 1, at = xtick+13, labels = xtlab, lwd.ticks=0)
axis(side = 1, at = xtick, labels = rep("",13))
...
### REDO PLOTS WITH RELATIVE SENSTIVITIES
...
pdf(file="D:/R/Climate_sensitivity/Figure S2cd.pdf",width=8, height=4)
...
# plot coefficients based on max variation
# melev <- max(height)-min(height)
# mglac <- max(glac)
# 
# if (abs == 1){ylim1 <- c(-.5,2.0)} else {ylim1 <- c(-.2,.3)}
# 
# plot(xx,mod_T$elev_c*melev, typ="b", ylim=ylim1, col="orange", xlab="", ylab="Temperature Sensitivity", xaxt = "n" )
# axis(side = 1, at = xtick+13, labels = xtlab, lwd.ticks=0)
# axis(side = 1, at = xtick, labels = rep("",13))
# abline(0,0)
# sigmod <- which(mod_T$elev_p < 0.05)
# points(xx[sigmod],mod_T$elev_c[sigmod]*melev, pch=16, col="orange", cex=1.5)
# 
# points(xx,mod_T$glac_c*mglac, typ="b", col="cyan")
# sigmod <- which(mod_T$glac_p < 0.05)
# points(xx[sigmod],mod_T$glac_c[sigmod]*mglac, pch=16, col="cyan", cex=1.5)
# 
# if(inter == 1){	 
#   points(xx,mod_T$inter_c*mglac, typ="b", col="blue")
#   sigmod <- which(mod_T$inter_p < 0.05)
#   points(xx[sigmod],mod_T$inter_c[sigmod]*mglac, pch=16, col="blue", cex=1.5)
# }



# plot(xx,mod_P$elev_c*melev, typ="b", ylim=c(-1,.5), col="orange", xlab="", ylab="Precipitation Sensitivity", xaxt = "n")
# axis(side = 1, at = xtick+13, labels = xtlab, lwd.ticks=0)
# axis(side = 1, at = xtick, labels = rep("",13))
# abline(0,0)
# sigmod <- which(mod_P$elev_p < 0.05)
# points(xx[sigmod],mod_P$elev_c[sigmod]*melev, pch=16, col="orange", cex=1.5)
# 
# points(xx,mod_P$glac_c*mglac, typ="b", col="cyan")
# sigmod <- which(mod_P$glac_p < 0.05)
# points(xx[sigmod],mod_P$glac_c[sigmod]*mglac, pch=16, col="cyan", cex=1.5)
